...
const SPECIAL_COMBOS = [
  {
    ...
    bonus: "☕ Combo kinh điển! Hương nhài hoà quyện trà xanh tạo nên thức uống thanh tao, nhẹ nhàng.",
  ...
  },
  {
    ...
    bonus: "💪 Siêu combo miễn dịch! Gừng ấm + chanh vitamin C + trà xanh chống oxy hóa.",
...
const COMPATIBILITY_MAP = {
  ...
  "honey": ["green-tea", "oolong", "black-tea", "white-tea", "ginger", "lemon", "rose"],
};
    ...
    if (ing.conflictsWith) {
      ing.conflictsWith.forEach((conflictId) => {
        if (selectedIds.includes(conflictId)) {
          const conflictIng = selectedIngredients.find((i) => i.id === conflictId);
          if (conflictIng) {
...
function getConflictReason(id1, id2) {
  ...
  const reasons = {
    "black-tea+green-tea": "Vị chát đắng chồng chất, caffeine cộng dồn quá cao, hương bị lẫn lộn.",
    "black-tea+white-tea": "Trà đen quá mạnh sẽ hoàn toàn lấn át vị thanh nhẹ của trà trắng.",
    "green-tea+white-tea": "Cả hai đều nhẹ nhưng hương vị tương tự — không tạo sự khác biệt.",
    ...
    "cinnamon+mint": "Quế nóng ấm vs bạc hà mát lạnh — tác dụng nhiệt đối nghịch nhau.",
  };
